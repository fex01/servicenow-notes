...
- Service Mapping Business Value
  ...
  - addressed business cases:
    - Enhanced Visibility: Accurate maps of IT services and their underlying infrastructure, enabling a real-time, comprehensive view of all components and dependencies
      - Reduce Mapping Efforts
        - avg. manual mapping time: 2 weeks
        - with SM support: 1-4 days
      - Reduce Mapping Maintenance Efforts
    - Improved Incident and Problem Management: Understanding service dependencies enables faster root cause identification, reducing downtime
    ...
    - Reduce Risk of Change
      ...
      - Insight into potential change impacts allows minimal-risk execution
    - Enhanced Compliance and Governance: Clear documentation of dependencies and data flow supports audits and regulatory compliance
...
- Service Mapping
  - Maps all software and hardware components that support a given service
  - Agentless, service-centric, configuration-based (not dependent on network traffic)
  - Top-down approach: maps only CIs supporting an entry point, not the full inventory
- Service examples
  - Email Service: Unix Server, AD/LDAP, Storage/SAN, Network devices, ESX server
  - Internal Web Service: IIS, Windows Server, Load Balancer, MSSQL, Storage, Network
  - Database Service: Unix, ESX, Storage
  - Stock Trader Service: IIS, Windows Server, MSSQL
- ITOM Solutions and Business Value: see [SMA Introduction](#sma-introduction)
- Architecture
  - Requires MID server (see [SNow MID Server](./sn-discovery-mid-server.md))
- Phases
  - Entry Point: e.g., URL, TCP connection, Citrix entry point
  - Discovery: targeted port scan, classification, identification, exploration
  - Process Detection: determine process behind entry point
  - Application Identification/Classification: pattern identification section
  - Connection: discover outgoing connections → new entry point
- approaches to populate Application Services
  - **Pattern-based Connections** – mission-critical apps; granular process-to-process mapping via patterns
  - **ML Connection Suggestions** – homegrown/custom COTS; traffic-based with application fingerprinting; faster time-to-value
  - **Dynamic CI Group** – small applications grouped by CI filters or CMDB queries; quick setup; no license required
  - **Tag-based** – cloud-native, container, or VM environments; uses tag categories/families for visualization of resource stacks
...
- CMDB data population target: **90% automated** via Service Mapping Discovery, **10% manual** with governance
- **Step 1 – Discover Infrastructure and Applications**:
  - Discover across all locations: data center, cloud, containers, microservices, serverless
- **Step 2 – Discover Services**:
  - Gain business context by mapping infrastructure/applications to business services
- **Step 3 – Unify Configuration Data**:
  - Create a central CMDB as the system of record for configuration data
- Apply governance at each step to maintain CMDB health and ensure reliable data
